...
// interposer.ini placed next to the DLL.
// Uses absolute paths (no %ENVVAR%) so LoadConfig and the test runner
// both resolve to exactly the same files regardless of %TMP% vs %TEMP%.
//
// The ini key is an ECMAScript regex pattern.  Each `\\` in the ini text
// represents one literal backslash in the regex engine, matching `\` in
// the Windows path.
static void WriteInterposerIni(const std::wstring& iniPath,
                                ...
                                const std::wstring& redirectBaseDir)
{
    ...
    std::string content =
        "[Settings]\r\n"
        "LogFile=" + WideToUtf8(logPath) + "\r\n"
        "LogFiles=1\r\n"
        "LogRegistry=1\r\n"
        "\r\n"
        "[FileRedirects]\r\n"
        // Pattern (ECMAScript regex stored verbatim as the ini key):
        //   C:\\TestGame\\Saves\\(.+)
        // Regex semantics: C:\TestGame\Saves\<capture group 1>
        // In this C++ string literal, \\\\ → \\ in the file → one regex backslash
        "C:\\\\TestGame\\\\Saves\\\\(.+)=" + WideToUtf8(redirectBaseDir) + "$1\r\n";
...
int wmain()
{
    ...
    // Log goes next to the DLL — avoids %TMP% vs %TEMP% divergence between
    // GetTempPathW() (used by the test) and ExpandEnvironmentStringsW (used by LoadConfig).
    std::wstring logPath    = exeDir    + L"interposer_test.log";
    std::wstring regPath    = exeDir    + L"VirtualRegistry.reg";
    std::wstring iniPath    = exeDir    + L"interposer.ini";
    std::wstring dllPath    = exeDir    + L"LANCommander.Interposer.dll";
    ...
    // Delete stale log from a prior run
    DeleteFileW(logPath.c_str());

    // Create the redirect-target directory and a file for redirect tests.
    // Also creates testTmpDir itself (needed for the log file later).
    std::wstring redirectDir    = testTmpDir + L"Redirected\\";
    std::wstring redirectTarget = redirectDir + L"profile.dat";
    CreateDirs(redirectDir);
    WriteTextFile(redirectTarget, "redirect_target_content");

    // interposer.ini must be in place before LoadLibraryW
    // (LoadConfig runs in DLL_PROCESS_ATTACH, before MH_EnableHook)
    WriteInterposerIni(iniPath, logPath, redirectDir);

    // VirtualRegistry.reg must be in place before LoadLibraryW
    ...
    // Leave interposer.ini and VirtualRegistry.reg for post-run inspection
